...
% 论文题目：三号黑体居中（16pt）
% 一级标题：四号黑体居中（14pt）
% 二级、三级标题：小四号黑体左对齐（12pt）
% 正文中文：小四号宋体（12pt，由 \documentclass[12pt] 自动生效，宋体为 ctex 默认）
% 行距：单倍行距
% 全文不设页眉，仅保留页脚页码（plain）
...
\textbf{问题一}（$n=15$ 单车 TSP）采用位置 one-hot 编码 $x_{i,p}\!\in\!\{0,1\}$ 构造 $n^{2}=225$ 比特 QUBO；Held-Karp 动态规划取得精确基线 $J=29$，SDK 模拟退火亦达 $J=29$，CIM 真机两次重测均收敛至 $J=31$（gap $+6.9\%$），完成小规模实例上的编码正确性与真机链路可行性验证。

\textbf{问题二}（同规模 TSPTW）保持 225 比特 QUBO 仅承担路径排序子问题，时间窗惩罚 $P_i=M_1[a_i\!-\!t_i]_+^2+M_2[t_i\!-\!b_i]_+^2$ 由解码后统一 evaluate 函数计算并作为混合搜索的业务评价目标；Python LNS$+$SA、SDK SA、CIM 真机三方在该实例上均得到 $J=84121$（travel $=31$, penalty $=84090$），作为本实例的近似最优可行解。

\textbf{问题三}（$n=50$ 单车 TSPTW）若整体编码需 $50^{2}=2500$ 比特，超过 550 比特真机规模上限，故采用\textbf{滚动窗口分解}将路径切为 3 段子 TSP，每段 $\le 17^{2}=289$ 比特；子问题分别交 SDK SA 与 CIM 真机求解后，由统一 evaluate 函数对完整 50 客户路径重新评价，Python LNS、SDK SA、CIM 真机第 3 段均达到 $J=4{,}941{,}906$。

\textbf{问题四}（$n=50$ 多车 CVRPTW，容量 $C=60$，总需求 $\sum d_i=271$ 给出下界 $K_{\min}=\lceil 271/60\rceil=5$）采用``\textbf{Clarke-Wright Savings 启发式分配 $+$ 每车独立子 TSP QUBO}''的分层编码（每子 QUBO $\le 8^{2}=64$ 比特），辅以跨车 LNS 与多 seed SA 元启发式；扫描 $K\!\in\![5,10]$ 取得最优车辆数 $K^{*}=7$，三方综合目标 $J=7149$（Python/SDK 一致）与 $J=7168$（CIM 真机，gap $+0.27\%$），$K=8$ block-diag 方案作为节省真机配额的补充验证。
...
电商与即时配送的快速发展，使城市物流面临``客户规模大、时间窗严苛、车辆资源有限''的复合调度难题；与此同时，玻色量子推出的 CPQC-550 相干伊辛机（以下简称 CIM 真机）\cite{boson2024cpqc} 支持 $\le 550$ 个 Ising 自旋的 QUBO 求解（其原理与器件实现可参考相干伊辛机综述\cite{yamamoto2017cim,honjo2021cim2000}），配套 Kaiwu SDK\cite{kaiwusdk2024} 提供模拟退火与真机算力的统一接口。本文不属于量子物理研究，而是以\textbf{智慧物流路径规划为应用背景}，研究在严格的量子比特预算下将路径规划转化为 QUBO，并通过 SDK 模拟退火与 CIM 真机进行求解与验证。
...
\textbf{问题四}：直接 3D one-hot $x_{i,p,k}$ 需 $K\!\cdot\!n^{2}$ 比特（$K_{\min}\!=\!5$ 时为 $12{,}500$ 比特，主方案 $K^{*}\!=\!7$ 时为 $17{,}500$ 比特），均远超 550 比特上限，不可行。本文采用``\textbf{Clarke-Wright Savings 做客户分配（启发式）$+$ 每车独立子 TSP QUBO}''的分层编码，每子 QUBO $\le 8^{2}=64$ 比特；车辆数与运输时间/时间窗惩罚之间的目标加权由综合目标 $J=\text{travel}+\sum P_i+M\!\cdot\!K$ 直接处理，并扫描 $K$ 给出 Pareto 前沿。$K=8$ 的 block-diag 一次提交方案作为\textbf{节省真机配额的补充验证策略}使用，不替代主分层模型本身。
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        algo/.style={rectangle, draw=orange!80, fill=orange!12, rounded corners=2pt,
                     minimum width=92mm, minimum height=9mm, align=center},
        ...
        \node[model, below=of raw, yshift=-2mm, xshift=18mm] (q1) {Q1: 单车 TSP\\225 比特};
        \node[model, right=of q1] (q2) {Q2: TSPTW\\225 比特$+$软罚};
        \node[model, below=of q1, yshift=-2mm] (q3) {Q3: 大规模 TSPTW\\滚动分段 $\le 289$};
        \node[model, right=of q3] (q4) {Q4: 多车 CVRPTW\\每车 $\le 64$};
        \node[algo, below=of q3, yshift=-2mm, xshift=18mm] (alg) {三层求解栈：Python 基线 $\to$ Kaiwu SDK SA $\to$ CIM 真机（$\le 550$ 比特, 8-bit）};
        \node[outbox, below=of alg, yshift=-2mm, xshift=-32mm] (sens) {灵敏度分析\\Q1: $A\!\times\!T_0$；Q4: $K$};
        ...
        \draw[arrow] (q2) -- (q4);
        \draw[arrow] (q3) -- (q4);
        \draw[arrow] (q3.south) -- ([xshift=-22mm]alg.north);
        \draw[arrow] (q4.south) -- ([xshift=22mm]alg.north);
        \draw[arrow] ([xshift=-30mm]alg.south) -- (sens.north);
        \draw[arrow] (alg.south) -- (cmp.north);
        \draw[arrow] ([xshift=30mm]alg.south) -- (concl.north);
    \end{tikzpicture}
...
为将式~\eqref{eq:q1-classical} 转化为 QUBO 形式（QUBO 形式化框架与 NP 问题 Ising 转换可参考\cite{lucas2014ising,glover2019qubo}；TSP 类组合优化问题在量子退火上的求解可参考\cite{warren2017tspqubo}），引入\textbf{位置 one-hot 编码}：
\begin{equation}
...
转换\cite{kadowaki1998qa,kaiwusdk2024}：通过 $x_{i,p}=(1-\sigma_{l})/2$ 的标准代换，并引入 1 个辅助自旋 $\sigma_{\rm aux}$
...
SDK 验证$\to$真机求解''递进展开：
\begin{enumerate}
  \item[(i)] \textbf{Held--Karp 动态规划}\cite{heldkarp1962dp}（精确基线）：在 $n=15$ 规模下直接求解
    式~\eqref{eq:q1-classical}，作为 QUBO 求解的正确性参考；时间复杂度 $\mathcal{O}(n^{2}2^{n})$。
  \item[(ii)] \textbf{Kaiwu SDK 模拟退火}\cite{kirkpatrick1983sa,kaiwusdk2024}（SDK 验证）：对式~\eqref{eq:q1-qubo} 转化得到的
...
\end{enumerate}
三档求解器返回的``近似可行''候选解统一交由 2-opt\cite{lin1973twoopt} $+$ Or-opt\cite{or1976oropt} 混合局部搜索进行精修，构成
...
问题二在问题一的单车辆 TSP 基础上引入\textbf{软时间窗惩罚}（即标准 TSPTW\cite{solomon1987vrptw}）：每个客户 $i$ 给定服务窗
...
\begin{enumerate}
  ...
  \item[(ii)] \textbf{纯 Python v2（多起点 $+$ LNS $+$ 多 seed SA）}：27 个 warm-start，
    150 轮 LNS\cite{shaw1998lns,ropke2006alns}（ruin-and-recreate $+$ regret-2 重插），3 个 SA 种子，配合 3-opt 重型精修。
...
逐客户的服务开始时刻、时间窗与惩罚情况见附录~\ref{app:q2-schedule}~中的表~\ref{tab:q2_schedule}（为压缩正文篇幅，详表移至附录），
对应的甘特图见图~\ref{fig:q2-gantt}。
...
\textbf{车辆数量决策}（即标准 CVRPTW\cite{toth2014vrp,solomon1987vrptw}；本文方法属于量子退火 / 经典启发式混合求解，相关工作可参考\cite{feld2019cvrp,salehi2022vrpqubo,harwood2021vrptwqubo}）：每辆车容量上限 $C=60$（题目附件 2 给定），
...
由于 $K\ge 5$、每车 7--8 客户的全 3D one-hot 编码 $x_{i,p,k}$ 需要 $K\!\cdot\!n^2$
比特（$K_{\min}\!=\!5$ 时为 $12\,500$，主方案 $K^{*}\!=\!7$ 时为 $17\,500$），远超 CPQC-550
真机 550 比特上限，本文沿用问题三的``经典启发式提供分配 $\to$
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccl}
        ...
        \midrule
        全 3D one-hot $x_{i,p,k}$                       & $K\!\cdot\!n^{2}=17\,500$ & 否 & 否          & 直接编码不可行 \\
...
由表~\ref{tab:q4-budget} 可知，$K\!\cdot\!n^{2}$ 全 3D one-hot 编码（$K^{*}\!=\!7$ 时
$17\,500$ 比特）超出真机比特预算约 32 倍，不可上传；将客户分配 $y_{i,k}$ 与车内排序联合
QUBO 化（$\sim 700$ 比特）仍超 550 上限。
\textbf{本文主方案}采用分层编码：① 客户分配 $y_{i,k}$ 由 6 种经典启发式（Clarke--Wright Savings\cite{clarke1964savings}、
...
分层 QUBO 编码绕开 $K\!\cdot\!n^{2}=17\,500$ 比特（$K^{*}\!=\!7$）的全 3D one-hot 不可行性，使每车子 QUBO
...
\textbf{针对问题四}（$n\!=\!50$ 多车 VRPTW）：采用启发式分配 $+$ 每车独立子 TSP QUBO（$\le\!64$ 比特）的分层编码，绕开 $K^{*}\!=\!7$ 全 3D one-hot $17\,500$ 比特直接编码的不可行性，求得近似最优 $J\!=\!7149$（$K^{\ast}\!=\!7$，$travel\!=\!109$，$penalty\!=\!40$）；CIM 真机 $K\!=\!7$ 得 $J\!=\!7168$（gap $+0.27\%$），Python/SDK/CIM 结果接近。$K\!\in\![5,10]$ 灵敏度显示本文在 $K\!\in\!\{5,6,8,9\}$ 严格优于参考，$K\!=\!7$ 持平；$K\!=\!8$ block-diag 副方案揭示了 ``配额 vs 解码质量'' 的真机部署 trade-off。
...
% =============================================================
%  参考文献：thebibliography 环境会自动输出"参考文献"标题；
%  为避免与手动 \section* 重复（旧版 PDF 中曾出现"参考文献 参考文献"），
%  此处不再额外写 \section*{参考文献}。
% =============================================================
\begin{thebibliography}{99}

% 按正文中引用次序排列（附件 1 规范要求）

\bibitem{boson2024cpqc}
玻色量子. CPQC-550 相干伊辛机用户手册（Kaiwu SDK 1.3.1）[Z/OL]. 北京: 北京玻色量子科技有限公司, 2024. \url{https://platform.qboson.com}, 访问时间：2026-04-28.
...
玻色量子. Kaiwu SDK 1.3.1 用户手册（含 SimulatedAnnealingOptimizer / CIMOptimizer / qubo\_matrix\_to\_ising\_matrix / adjust\_qubo\_matrix\_precision API 文档）[Z/OL]. 北京: 北京玻色量子科技有限公司, 2024. \url{https://platform.qboson.com}, 访问时间：2026-04-28.

\bibitem{problem2026}
2026 年全国大学生数学建模竞赛校园选拔赛 A 题（含附件：参考算例.xlsx 等）[Z]. 2026.
...
% =============================================================
%  附录（按附件 1 规范：放在参考文献之后；A 支撑材料文件列表、
%  B 运行环境与复现入口、C 核心源程序文件说明、D 真机日志与中间结果说明、
%  E 问题二逐客户调度明细表）
% =============================================================
\appendix
% 附录段后重新配置 CTeXsetup：标题形如"附录 A 支撑材料文件列表"，
% \ref{} 输出对应字母（A/B/C/D/E）
...
支撑材料以 ZIP 形式与本论文同步提交，目录结构如下；其中``建模所用到的全部完整、
可运行的源程序代码''集中放在 \texttt{src/} 下，可由附录~\ref{app:env}~与附录~\ref{app:src}~的
入口与文件说明定位：

\begin{itemize}[leftmargin=2em,itemsep=0.2ex]
  \item \texttt{src/}：四问全部可运行的 Python 源程序（\texttt{\_q\_lib.py} 共用工具，
        \texttt{01\_q1\_*.py}--\texttt{15b\_q4\_*\_blockdiag\_*.py} 各阶段脚本，
        \texttt{13\_all\_q\_cim\_real.py} CIM 真机调用入口）；
  \item \texttt{data/}：题目附件原始数据（\texttt{参考算例.xlsx} 只读）与本文清洗 / 衍生数据；
  \item \texttt{results/基础模型/}：四问 Python 与 Kaiwu SDK 求解结果 JSON
        与 QUBO 矩阵（\texttt{qubo\_matrices/} 含 8-bit 整数 dense CSV）；
  \item \texttt{results/真机结果/}：CPQC-550 真机 12 次任务的 spin 解 zip、
        解码 JSON、哈密顿量 PDF 与平台原始日志，并附四问真机对比汇总表；
  \item \texttt{results/灵敏度分析/}：Q1 罚系数 $A\!\times\!T_0$ 网格、
        Q4 车辆数 $K$ 灵敏度的 CSV / JSON；
  \item \texttt{figures/}：论文中所有 PDF 矢量图与对应 PNG（dpi $\ge 300$）；
  \item \texttt{tables/}：论文中所有 CSV 数据表与 LaTeX 三线表源；
  \item \texttt{paper/Manuscript/}：本文 LaTeX 主稿与子目录章节源（\texttt{main.tex}
        为唯一编译入口，xelatex / latexmk -xelatex 即可重现 PDF）；
  \item \texttt{paper/章节/}：实验日志（\texttt{00\_实验日志\_问题<X>.md}）与
        论文素材库（\texttt{00\_论文素材库\_问题<X>.md}）；
...
\begin{itemize}[leftmargin=2em,itemsep=0.2ex]
  \item 操作系统：Windows 11（64 位）或同等 Linux / macOS 64 位；
  \item Python：\textbf{Python 3.10}（必需；Kaiwu SDK 1.3.1 的发行 wheel 仅支持
        \texttt{cp310-none-win\_amd64}，本工程压缩包内附 \texttt{kaiwu-1.3.1-cp310-none-win\_amd64.whl}）；
  \item 关键依赖：\texttt{kaiwu==1.3.1}、\texttt{numpy}、\texttt{pandas}、
        \texttt{matplotlib}、\texttt{openpyxl}、\texttt{tqdm}（详见支撑材料 \texttt{requirements.txt}）；
  \item LaTeX 编译：XeLaTeX（中文经 ctex 宏包，建议 TeX Live 2023 及以上），
        编译命令 \texttt{xelatex main.tex} 或 \texttt{latexmk -xelatex main.tex}；
  \item CIM 真机：玻色 CPQC-550 平台（\url{https://platform.qboson.com}），
        通过 Kaiwu SDK \texttt{CIMOptimizer} 接口或网页手动上传 8-bit 整数 CSV
        矩阵（每次任务占 1 次配额，矩阵 ising 维度 $\le 550$）。
\end{itemize}

\textbf{复现入口}（在仓库根目录下执行；$\langle$\,project\_root\,$\rangle$ 为支撑材料解压后所在目录）：

\begin{itemize}[leftmargin=2em,itemsep=0.2ex]
  \item 全流程：\texttt{python src/00\_run\_all.py}（依次跑 Q1--Q4 Python 基线
        与 SDK SA，结果落 \texttt{results/基础模型/}）；
  \item 单问入口：\texttt{python src/01\_q1\_pure\_python.py}、
        \texttt{python src/05\_q2\_pure\_python.py}、
        \texttt{python src/05c\_q2\_pure\_python\_v2.py}、
        \texttt{python src/09\_q3\_kaiwu\_decompose.py}、
        \texttt{python src/14\_q4\_pure\_python.py}；
  \item CIM 真机调用：\texttt{python src/13\_all\_q\_cim\_real.py}
        （含 token / 矩阵上传 / spin 解下载 / 哈密顿量绘图，需平台账号）；
  \item 灵敏度：\texttt{python src/06\_q1\_sens\_A\_T0.py}、
        \texttt{python src/14b\_q4\_K\_sensitivity.py}；
  \item 图表生成：\texttt{python src/draw\_*.py}（输出落 \texttt{figures/} 与 \texttt{tables/}）。
\end{itemize}

\section{核心源程序文件说明}
\label{app:src}

为节省正文篇幅，本附录仅列出建模与求解链路上的核心文件清单及其职责。完整、
可运行源程序代码全部以原文件形式随支撑材料 ZIP 一同提交（\texttt{src/} 目录下，
逐脚本顶部含 docstring 与命令行用法）。

\begin{itemize}[leftmargin=2em,itemsep=0.2ex]
  \item \texttt{src/\_q\_lib.py}：数据读取（\texttt{load\_data}）、距离矩阵、
        服务时间递推、统一 \texttt{evaluate}、$P_i(t_i)$ 早晚到二次罚、
        2-opt / Or-opt / Swap 修复算子——四问全部脚本共用此一份口径；
  \item \texttt{src/01\_q1\_pure\_python.py}：Q1 Held--Karp 精确动态规划基线；
  \item \texttt{src/02\_q1\_qubo\_kaiwu\_sdk.py}：Q1 位置 one-hot QUBO 构造、
        Kaiwu SDK 模拟退火求解、\texttt{adjust\_qubo\_matrix\_precision} 8-bit 调整、
        Ising 矩阵导出 CSV；
  \item \texttt{src/05\_q2\_pure\_python.py} / \texttt{05c\_q2\_pure\_python\_v2.py}：
        Q2 多起点启发式 + LNS + 多 seed SA 求解 TSPTW；
  \item \texttt{src/09\_q3\_kaiwu\_decompose.py}：Q3 滚动窗口分解 subQUBO（17/17/16）
        与 \texttt{solve\_subqubo(Q, backend)} 抽象层；
  \item \texttt{src/14\_q4\_pure\_python.py}：Q4 多起点 + 跨车 LNS + 单车 polish + $K$ 扫描；
  \item \texttt{src/15b\_q4\_k8\_blockdiag\_cim.py}：Q4 $K=8$ block-diag 7 段
        合并子 QUBO 配额优化方案；
  \item \texttt{src/13\_all\_q\_cim\_real.py}：CIM 真机统一调用入口
        （token 配置、矩阵上传、spin 解下载、哈密顿量曲线绘图、与 SDK SA 的对照）；
  \item \texttt{src/06\_q1\_sens\_A\_T0.py} / \texttt{14b\_q4\_K\_sensitivity.py}：
        两类灵敏度扫描；
  \item \texttt{src/draw\_*.py}：图表生成脚本，输出落到 \texttt{figures/} 与 \texttt{tables/}。
\end{itemize}

\section{真机日志与中间结果说明}
\label{app:cim}

本文累计向 CPQC-550 真机提交 \textbf{12 次任务}，所有提交矩阵的 ising 维度均
$\le 550$ 比特，且经 \texttt{adjust\_qubo\_matrix\_precision(Q, bit\_width=8)}
8-bit 精度调整后上传。各任务的输入 / 返回与解码结果落盘于 \texttt{results/真机结果/}。
任务清单见表~\ref{tab:cim-tasks}。
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c c l}
        \toprule
        任务编号 & 对应问题 & ising 维度 & 备注 & 产出文件路径（\texttt{results/真机结果/}下） \\
        \midrule
        ...
        R-Q3-seg2 & Q3 段 2            & 290           & 单 task 10 sample & \texttt{q3/cim\_q3\_seg2\_*.zip / *.json / *.pdf} \\
        R-Q3-seg3 & Q3 段 3            & 257           & 单 task 10 sample & \texttt{q3/cim\_q3\_seg3\_*.zip / *.json / *.pdf} \\
        R-Q4-V1$\sim$V7 & Q4 $K^{*}\!=\!7$ 七车 & $\le 65$/车 & 共 7 次配额，每车 10 sample & \texttt{q4/cim\_q4\_v\{1..7\}\_*.zip} 等 \\
        R-Q4-K8-bd & Q4 $K\!=\!8$ block-diag & 298 & v2--v8 合并 1 次配额    & \texttt{q4/cim\_q4\_k8\_blockdiag\_*.zip} \\
        \bottomrule
    \end{tabular}
\end{table}

由表~\ref{tab:cim-tasks} 可知：（i）所有真机任务的 ising 维度均不超过 550，可直接
通过平台手动上传或 SDK \texttt{CIMOptimizer} 提交；（ii）Q3 段 1 因旧账号配额异常未
获得有效返回，本文未补造数据，而以 \texttt{seg1\_failure.log} 在结果目录中如实标注；
（iii）每个真机 spin 解的长度 $=$ ising 维度 $=$ QUBO 维度 $+$ 1（含 1 个辅助自旋），
解码取 \texttt{spin[:nvar]}，最后 1 位辅助维丢弃，详见 \texttt{src/13\_all\_q\_cim\_real.py}；
（iv）Q2 / Q3 / Q4 详细哈密顿量演化曲线 PDF 与原始 spin 解 zip 均位于
\texttt{results/真机结果/} 对应子目录下，可与正文图~\ref{fig:q1-cim-h}/\ref{fig:q2-cim-h}/\ref{fig:q3-cim-h}/\ref{fig:q4-cim-h}
原始数据交叉核验。

\section{问题二逐客户调度明细表}
\label{app:q2-schedule}

为压缩正文篇幅，问题二 15 客户的逐点服务开始时刻、时间窗 $[a_i,b_i]$ 与单点惩罚
$P_i$ 详表（与正文表~\ref{tab:q2_schedule} 对应）移至本附录；正文 §\ref{subsec:q2-result}
仅保留汇总数值与甘特图。
